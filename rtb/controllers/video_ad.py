...
def apiSendVideoCampaingData(request):
    ...
    for row in queryRes:
        answer["campaigns"].append({
            ...
            "fill_rate_hour": 0 if row.fill_rate_hour is None else row.fill_rate_hour*100,
        ...
        if answer["campaigns"][-1]["sum_imps"] == 0:
            ...
            answer["campaigns"][-1]["fill_rate"] = float(answer["campaigns"][-1]["ad_starts"]) / \
                                                   answer["campaigns"][-1]["sum_imps"] * 100
        answer["campaigns"][-1]["profit_loss"] = float(answer["campaigns"][-1]["cpm"]) * answer["campaigns"][-1][
            "sum_imps"] - float(answer["campaigns"][-1]["cpvm"]) * answer["campaigns"][-1]["ad_starts"]
