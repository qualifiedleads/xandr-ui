...
insert into ui_usual_placements_grid_data_""" + str(type) + """_tracker as ut (
  ...
  select
    ...
    "PlacementId",
    count(id) as imps,
    coalesce(sum("PricePaid"),0) / 1000.0 as spent,
    0,
    0,
    case coalesce(count(id), 0) when 0 then 0 else (coalesce(sum("PricePaid"),0) / 1000.0) / count(id) * 1000.0 end,
    0,
    0,
    0,
    0,
...
insert into ui_usual_placements_grid_data_last_month_tracker as ut (
  ...
  coalesce(sum(t."PricePaid"),0) / 1000.0 as spend,
  count(conversiontable.id) as conversions,
  0 as imps_viewed,
  0 as view_measured_imps,
  case count(t."id") when 0 then 0 else (coalesce(sum(t."PricePaid"),0)  / 1000.0) / count(t."id") * 1000.0 end cpm,
  ...
  case count(clicktable.id) when 0 then 0 else (coalesce(sum(t."PricePaid"),0) / 1000.0) / count(clicktable.id) end cpc,
  case count(conversiontable.id) when 0 then 0 else (coalesce(sum(t."PricePaid"),0) / 1000.0) / count(conversiontable.id) end cpa,
...
insert into ui_usual_campaigns_grid_data_""" + str(type) + """_tracker as ut (
  ...
  select
    ...
    array_to_json(array((select
         ...
         'imp', count(site_r."id"),
         'spend', coalesce(sum(site_r."PricePaid"),0) / 1000.0,
         'clicks', 0,
         'conversions', 0,
         'cvr', 0,
         'cpc', 0,
...
insert into ui_usual_campaigns_grid_data_last_month_tracker as ut (
  ...
  array_to_json(array((select
         ...
         'cpc', case count(clicktable.id) when 0 then 0 else (coalesce(sum(site_r."PricePaid"),0) / 1000.0)::float/count(clicktable.id) end,
...
from (
      select
        ...
        count(conversiontable.id) as conversions,
        count(t."id") as imps,
        coalesce(sum(t."PricePaid"),0) as spend,
        0 as imps_viewed,
        0 as view_measured_imps,
        case count(t."id") when 0 then 0 else coalesce(sum(t."PricePaid"),0) / count(t."id") * 1000.0 end cpm,
        case count(t."id") when 0 then 0 else count(conversiontable.id)::float / count(t."id") end cvr,
        case count(t."id") when 0 then 0 else count(clicktable.id)::float / count(t."id") end ctr,
        case count(clicktable.id) when 0 then 0 else coalesce(sum(t."PricePaid"),0) / count(clicktable.id) end cpc,
        0 as view_measurement_rate,
        0 as view_rate
      from
        rtb_impression_tracker t
...
insert into ui_usual_campaigns_graph_tracker as ut (
  ...
  array_to_json(array((select
         ...
         'mediaspent', (coalesce(sum(site_r."PricePaid"),0) / 1000.0),
         'clicks', 0,
         'conversions', 0,
         'cpa', 0,
         'cpc', 0,
...
insert into ui_usual_campaigns_graph_tracker as ut (
  ...
  select
      ...
      array_to_json(array((select
             ...
             'clicks', count(clicktable.id),
             'conversions', count(conversiontable.id),
             'cpa', case count(conversiontable.id) when 0 then 0 else (coalesce(sum(site_r."PricePaid"),0) / 1000.0)/count(conversiontable.id) end,
             'cpc', case count(clicktable.id) when 0 then 0 else (coalesce(sum(site_r."PricePaid"),0) / 1000.0)::float/count(clicktable.id) end,
             'ctr', case count(site_r."id") when 0 then 0 else count(clicktable.id)::float/count(site_r."id") end)
           from
                rtb_impression_tracker site_r
