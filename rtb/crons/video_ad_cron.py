...
def fillVideoAdDataCron():
    ...
    last_date = VideoAdCampaigns.objects.aggregate(Max("hour"))
    if last_date["hour__max"] is None:
        last_date = datetime.strptime("2017 01 20 00:00:00", "%Y %m %d %H:%M:%S")
    else:
        last_date = last_date["hour__max"]

    after = last_date + timedelta(hours=1)
